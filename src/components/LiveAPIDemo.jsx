...
import { ConnectToHumanTool, ProcessRefundTool } from "../utils/tools";
...
const LiveAPIDemo = forwardRef(
  ({ onConnectionChange, onAudioStreamChange }, ref) => {
    ...
    const systemInstructions = `You are MAX, a level headed assistant. Always keep your voice tone depending on the question asked

Instructions:
1.Speak with calmness and ease 
2. If any question is asked about current affairs or date or weather etc, do a google search and provide the results
3. If any videos are requested, get those videos from youtube. Do a youtube search and provide the results back to the user
4. Ask if they would like to know more about it.
5. Sign off by saying "This is Max. I am your assistant and here if you need me anytime
`;
      ...
      switch (message.type) {
        ...
        case MultimodalLiveResponseType.TOOL_CALL: {
          ...
          functionCalls.forEach((functionCall) => {
            ...
            addMessage(`🛠️ Called tool: ${name}`, "tool-call");
    ...
    const connect = async () => {
      ...
      try {
        ...
        if (!enableGrounding) {
          // Register Sports Announcer Tools
          clientRef.current.addFunction(
            new ConnectToHumanTool((reason) => {
              setModalContent({
                title: "Connecting to Co-Commentator",
                message: `Reason: ${reason}\n\nStanding by for the handoff...`,
              });
          ...
          clientRef.current.addFunction(
            new ProcessRefundTool((params) => {
              setModalContent({
                title: "Instant Replay Triggered",
                message: `Play ID: ${params.transactionId}\nAnalysis: ${params.reason}`,
              });
              setModalVisible(true);
            })
          );
        ...
        <div className="main-content">
          <div className="info-panel">
            ...
            <p className="demo-intro">
              Experience MAX — your electrifying live sports announcer powered
              by Gemini. Talk to MAX, show him what you see, and let him turn
              every moment into an unforgettable broadcast.
            </p>
            <div className="info-item">
              <h4>Multimodal Commentary</h4>
              <p>
                Seamlessly processes audio and video input, letting MAX narrate
                exactly what he sees in real time like a live broadcast.
              </p>
            </div>
            <div className="info-item">
              <h4>Explosive Energy</h4>
              <p>
                Detects the mood of the moment and dials up the drama, tension,
                and excitement accordingly.
              </p>
            </div>
            <div className="info-item">
              <h4>Live Broadcast Tools</h4>
              <p>MAX can call real actions mid-broadcast:</p>
              <ul>
                <li>Instant Replay Analysis</li>
                <li>Connect to Co-Commentator</li>
              </ul>
            </div>
            <div className="info-item">
              <h4>Try Saying:</h4>
              <ul>
                <li>
                  "MAX, I just scored a goal!" (Let him narrate your moment)
                </li>
                <li>
                  "Look at this play!" (Show your camera for live commentary)
                </li>
                <li>
                  "Give me the post-match analysis." (Test analytical mode)
                </li>
                <li>"It's the final second!" (Test dramatic tension mode)</li>
              </ul>
            </div>
          </div>
            ...
            <div className="chat-container-main" ref={chatContainerRef}>
              {chatMessages.length === 0 && (
                <div className="empty-state">
                  <p>Connect to start the broadcast with MAX</p>
                  {!connected && (
